...
export const createResource = async (tipo: string, configuracion: string, estado: string, id_usuario: number) => {
  try {
    const query = `
      ...
      RETURNING *;
    `;
    const values = [tipo, configuracion, estado, id_usuario];
    const result = await pool.query(query, values);

    logger.info(`Recurso creado en la base de datos: ${JSON.stringify(result.rows[0])}`);
    return result.rows[0];
  } catch (error) {
    logger.error('Error al ejecutar la consulta de creación de recurso:', error);
